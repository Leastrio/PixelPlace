(() => {
  ...
  var require_topbar = __commonJS({
    "vendor/topbar.js"(exports, module) {
      (function(window2, document2) {
        ...
        var canvas2, currentProgress, showing, progressTimerId = null, fadeTimerId = null, delayTimerId = null, addEvent = function(elem, type, handler) {
          ...
          canvas2.width = window2.innerWidth;
          canvas2.height = options.barThickness * 5;
          var ctx2 = canvas2.getContext("2d");
          ctx2.shadowBlur = options.shadowBlur;
          ctx2.shadowColor = options.shadowColor;
          var lineGradient = ctx2.createLinearGradient(0, 0, canvas2.width, 0);
          for (var stop in options.barColors)
            lineGradient.addColorStop(stop, options.barColors[stop]);
          ctx2.lineWidth = options.barThickness;
          ctx2.beginPath();
          ctx2.moveTo(0, options.barThickness / 2);
          ctx2.lineTo(
            Math.ceil(currentProgress * canvas2.width),
            options.barThickness / 2
          );
          ctx2.strokeStyle = lineGradient;
          ctx2.stroke();
          ...
          if (options.className)
            canvas2.classList.add(options.className);
          document2.body.appendChild(canvas2);
          ...
          hide: function() {
            ...
            (function loop() {
              if (topbar2.progress("+.1") >= 1) {
                canvas2.style.opacity -= 0.05;
                if (canvas2.style.opacity <= 0.05) {
  ...
  var serializer_default = {
    ...
    binaryEncode(message) {
      ...
      let view2 = new DataView(header);
      let offset = 0;
      view2.setUint8(offset++, this.KINDS.push);
      view2.setUint8(offset++, join_ref.length);
      view2.setUint8(offset++, ref.length);
      view2.setUint8(offset++, topic.length);
      view2.setUint8(offset++, event.length);
      Array.from(join_ref, (char) => view2.setUint8(offset++, char.charCodeAt(0)));
      Array.from(ref, (char) => view2.setUint8(offset++, char.charCodeAt(0)));
      Array.from(topic, (char) => view2.setUint8(offset++, char.charCodeAt(0)));
      Array.from(event, (char) => view2.setUint8(offset++, char.charCodeAt(0)));
    ...
    binaryDecode(buffer) {
      ...
      let kind = view2.getUint8(0);
      let decoder = new TextDecoder();
      switch (kind) {
        case this.KINDS.push:
          return this.decodePush(buffer, view2, decoder);
        case this.KINDS.reply:
          return this.decodeReply(buffer, view2, decoder);
        case this.KINDS.broadcast:
          return this.decodeBroadcast(buffer, view2, decoder);
      }
    },
    decodePush(buffer, view2, decoder) {
      let joinRefSize = view2.getUint8(1);
      let topicSize = view2.getUint8(2);
      let eventSize = view2.getUint8(3);
    ...
    decodeReply(buffer, view2, decoder) {
      let joinRefSize = view2.getUint8(1);
      let refSize = view2.getUint8(2);
      let topicSize = view2.getUint8(3);
      let eventSize = view2.getUint8(4);
    ...
    decodeBroadcast(buffer, view2, decoder) {
      let topicSize = view2.getUint8(1);
      let eventSize = view2.getUint8(2);
  ...
  var debug = (view2, kind, msg, obj) => {
    if (view2.liveSocket.isDebugEnabled()) {
      console.log(`${view2.id} ${kind}: ${msg} - `, obj);
    }
  };
  ...
  var JS = {
    exec(eventType, phxEvent, view2, sourceEl, defaults) {
      ...
      commands.forEach(([kind, args]) => {
        ...
        this.filterToEls(sourceEl, args).forEach((el) => {
          this[`exec_${kind}`](eventType, phxEvent, view2, sourceEl, el, args);
        });
      });
    },
    isVisible(el) {
    ...
    exec_exec(eventType, phxEvent, view2, sourceEl, el, { attr, to }) {
      ...
      nodes.forEach((node) => {
        ...
        view2.liveSocket.execJS(node, encodedJS, eventType);
      });
    },
    exec_dispatch(eventType, phxEvent, view2, sourceEl, el, { to, event, detail, bubbles }) {
      detail = detail || {};
      detail.dispatcher = sourceEl;
      dom_default.dispatchEvent(el, event, { detail, bubbles });
    },
    exec_push(eventType, phxEvent, view2, sourceEl, el, args) {
      let { event, data, target, page_loading, loading, value, dispatcher, callback } = args;
      let pushOpts = { loading, value, target, page_loading: !!page_loading };
      let targetSrc = eventType === "change" && dispatcher ? dispatcher : sourceEl;
      let phxTarget = target || targetSrc.getAttribute(view2.binding("target")) || targetSrc;
      view2.withinTargets(phxTarget, (targetView, targetCtx) => {
    ...
    exec_navigate(eventType, phxEvent, view2, sourceEl, el, { href, replace }) {
      view2.liveSocket.historyRedirect(href, replace ? "replace" : "push");
    },
    exec_patch(eventType, phxEvent, view2, sourceEl, el, { href, replace }) {
      view2.liveSocket.pushHistoryPatch(href, replace ? "replace" : "push", sourceEl);
    },
    exec_focus(eventType, phxEvent, view2, sourceEl, el) {
      window.requestAnimationFrame(() => aria_default.attemptFocus(el));
    },
    exec_focus_first(eventType, phxEvent, view2, sourceEl, el) {
      window.requestAnimationFrame(() => aria_default.focusFirstInteractive(el) || aria_default.focusFirst(el));
    },
    exec_push_focus(eventType, phxEvent, view2, sourceEl, el) {
      window.requestAnimationFrame(() => focusStack.push(el || sourceEl));
    },
    exec_pop_focus(eventType, phxEvent, view2, sourceEl, el) {
    ...
    exec_add_class(eventType, phxEvent, view2, sourceEl, el, { names, transition, time }) {
      this.addOrRemoveClasses(el, names, [], transition, time, view2);
    },
    exec_remove_class(eventType, phxEvent, view2, sourceEl, el, { names, transition, time }) {
      this.addOrRemoveClasses(el, [], names, transition, time, view2);
    },
    exec_toggle_class(eventType, phxEvent, view2, sourceEl, el, { to, names, transition, time }) {
      this.toggleClasses(el, names, transition, time, view2);
    },
    exec_toggle_attr(eventType, phxEvent, view2, sourceEl, el, { attr: [attr, val1, val2] }) {
    ...
    exec_transition(eventType, phxEvent, view2, sourceEl, el, { time, transition }) {
      this.addOrRemoveClasses(el, [], [], transition, time, view2);
    },
    exec_toggle(eventType, phxEvent, view2, sourceEl, el, { display, ins, outs, time }) {
      this.toggle(eventType, view2, el, display, ins, outs, time);
    },
    exec_show(eventType, phxEvent, view2, sourceEl, el, { display, transition, time }) {
      this.show(eventType, view2, el, display, transition, time);
    },
    exec_hide(eventType, phxEvent, view2, sourceEl, el, { display, transition, time }) {
      this.hide(eventType, view2, el, display, transition, time);
    },
    exec_set_attr(eventType, phxEvent, view2, sourceEl, el, { attr: [attr, val] }) {
      this.setOrRemoveAttrs(el, [[attr, val]], []);
    },
    exec_remove_attr(eventType, phxEvent, view2, sourceEl, el, { attr }) {
      this.setOrRemoveAttrs(el, [], [attr]);
    },
    show(eventType, view2, el, display, transition, time) {
      if (!this.isVisible(el)) {
        this.toggle(eventType, view2, el, display, transition, null, time);
      }
    },
    hide(eventType, view2, el, display, transition, time) {
      if (this.isVisible(el)) {
        this.toggle(eventType, view2, el, display, null, transition, time);
      }
    },
    toggle(eventType, view2, el, display, ins, outs, time) {
    ...
    toggleClasses(el, classes, transition, time, view2) {
      window.requestAnimationFrame(() => {
        let [prevAdds, prevRemoves] = dom_default.getSticky(el, "classes", [[], []]);
        let newAdds = classes.filter((name) => prevAdds.indexOf(name) < 0 && !el.classList.contains(name));
        let newRemoves = classes.filter((name) => prevRemoves.indexOf(name) < 0 && el.classList.contains(name));
        this.addOrRemoveClasses(el, newAdds, newRemoves, transition, time, view2);
      });
    },
    addOrRemoveClasses(el, adds, removes, transition, time, view2) {
      time = time || default_transition_time;
      ...
      if (transitionRun.length > 0) {
        ...
        return view2.transition(time, onStart, onDone);
      }
      window.requestAnimationFrame(() => {
        let [prevAdds, prevRemoves] = dom_default.getSticky(el, "classes", [[], []]);
...
`);
  ...
  var UploadEntry = class {
    ...
    constructor(fileEl, file, view2, autoUpload) {
  ...
  var LiveUploader = class {
    ...
    constructor(inputEl, view2, onComplete) {
      this.autoUpload = dom_default.isAutoUpload(inputEl);
      this.view = view2;
      this.onComplete = onComplete;
      this._entries = Array.from(LiveUploader.filesAwaitingPreflight(inputEl) || []).map((file) => new UploadEntry(inputEl, file, view2, this.autoUpload));
  ...
  var DOMPatch = class {
    ...
    constructor(view2, container, id, html, streams, targetCID) {
      this.view = view2;
      this.liveSocket = view2.liveSocket;
      this.container = container;
      this.id = id;
      this.rootID = view2.root.id;
      this.html = html;
    ...
    perform(isJoinPatch) {
      let { view: view2, liveSocket: liveSocket2, container, html } = this;
      ...
      function morph(targetContainer2, source, withChildren = false) {
        morphdom_esm_default(targetContainer2, source, {
          ...
          onNodeAdded: (el) => {
            ...
            if (dom_default.isPhxChild(el) && view2.ownsElement(el) || dom_default.isPhxSticky(el) && view2.ownsElement(el.parentNode)) {
      ...
      liveSocket2.time("morphdom", () => {
        ...
        if (isJoinPatch) {
          dom_default.all(this.container, `[${phxUpdate}=${PHX_STREAM}]`, (el) => {
            this.liveSocket.owner(el, (view22) => {
              if (view22 === this.view) {
  ...
  var ViewHook = class {
    ...
    constructor(view2, el, callbacks) {
      this.__view = view2;
      this.liveSocket = view2.liveSocket;
    ...
    pushEventTo(phxTarget, event, payload = {}, onReply = function() {
      ...
      return this.__view.withinTargets(phxTarget, (view2, targetCtx) => {
        return view2.pushHookEvent(this.el, targetCtx, event, payload, onReply);
      });
    }
    ...
    uploadTo(phxTarget, name, files) {
      return this.__view.withinTargets(phxTarget, (view2, targetCtx) => {
        view2.dispatchUploads(targetCtx, name, files);
      });
    }
    __cleanup__() {
  ...
  var View = class {
    ...
    withinTargets(phxTarget, callback, dom = document, viewEl) {
      if (phxTarget instanceof HTMLElement || phxTarget instanceof SVGElement) {
        return this.liveSocket.owner(phxTarget, (view2) => callback(view2, phxTarget));
      }
      if (isCid(phxTarget)) {
        ...
        targets.forEach((target) => this.liveSocket.owner(target, (view2) => callback(view2, target)));
      }
    }
    ...
    joinChild(el) {
      ...
      if (!child) {
        let view2 = new View(el, this.liveSocket, this);
        this.root.children[this.id][view2.id] = view2;
        view2.join();
        this.childJoins++;
        return true;
      }
    ...
    onAllChildJoinsComplete() {
      ...
      this.formsForRecovery = {};
      this.joinCallback(() => {
        this.pendingJoinOps.forEach(([view2, op]) => {
    ...
    pushFileProgress(fileEl, entryRef, progress, onReply = function() {
      ...
      this.liveSocket.withinOwners(fileEl.form, (view2, targetCtx) => {
        view2.pushWithReply(null, "progress", {
          event: fileEl.getAttribute(view2.binding(PHX_PROGRESS)),
          ref: fileEl.getAttribute(PHX_UPLOAD_REF),
          entry_ref: entryRef,
          progress,
          cid: view2.targetComponentID(fileEl.form, targetCtx)
        }, onReply);
      });
    }
  ...
  var LiveSocket = class {
    ...
    execJS(el, encodedJS, eventType = null) {
      this.owner(el, (view2) => js_default.exec(eventType, encodedJS, view2, el));
    }
    execJSHookPush(el, phxEvent, data, callback) {
      this.withinOwners(el, (view2) => {
        js_default.exec("hook", phxEvent, view2, el, ["push", { data, callback }]);
      });
    }
    unload() {
      if (this.unloaded) {
    ...
    log(view2, kind, msgCallback) {
      if (this.viewLogger) {
        let [msg, obj] = msgCallback();
        this.viewLogger(view2, kind, msg, obj);
      } else if (this.isDebugEnabled()) {
        let [msg, obj] = msgCallback();
        debug(view2, kind, msg, obj);
      }
    }
    ...
    wrapPush(view2, opts, push) {
      let latency = this.getLatencySim();
      let oldJoinCount = view2.joinCount;
      if (!latency) {
        if (this.isConnected() && opts.timeout) {
          return push().receive("timeout", () => {
            if (view2.joinCount === oldJoinCount && !view2.isDestroyed()) {
              this.reloadWithJitter(view2, () => {
                this.log(view2, "timeout", () => ["received timeout while communicating with server. Falling back to hard refresh for recovery"]);
              });
            }
          });
        } else {
    ...
    reloadWithJitter(view2, log) {
      ...
      this.reloadWithJitterTimer = setTimeout(() => {
        if (view2.isDestroyed() || view2.isConnected()) {
          return;
        }
        view2.destroy();
        log ? log() : this.log(view2, "join", () => [`encountered ${tries} consecutive reloads`]);
        if (tries > this.maxReloads) {
          this.log(view2, "join", () => [`exceeded ${this.maxReloads} consecutive reloads. Entering failsafe mode`]);
        }
    ...
    joinDeadView() {
      ...
      if (body && !this.isPhxView(body) && !this.isPhxView(document.firstElementChild)) {
        let view2 = this.newRootView(body);
        view2.setHref(this.getHref());
        view2.joinDead();
        if (!this.main) {
          this.main = view2;
        }
        window.requestAnimationFrame(() => view2.execNewMounted());
      }
    }
    joinRootViews() {
      ...
      dom_default.all(document, `${PHX_VIEW_SELECTOR}:not([${PHX_PARENT_ID}])`, (rootEl) => {
        if (!this.getRootById(rootEl.id)) {
          let view2 = this.newRootView(rootEl);
          view2.setHref(this.getHref());
          view2.join();
    ...
    newRootView(el, flash, liveReferer) {
      let view2 = new View(el, this, null, flash, liveReferer);
      this.roots[view2.id] = view2;
      return view2;
    }
    owner(childEl, callback) {
      let view2 = maybe(childEl.closest(PHX_VIEW_SELECTOR), (el) => this.getViewByEl(el)) || this.main;
      if (view2) {
        callback(view2);
      }
    }
    withinOwners(childEl, callback) {
      this.owner(childEl, (view2) => callback(view2, childEl));
    }
    getViewByEl(el) {
    ...
    dropActiveElement(view2) {
      if (this.prevActive && view2.ownsElement(this.prevActive)) {
        this.prevActive = null;
      }
    }
    ...
    bindTopLevelEvents({ dead } = {}) {
      ...
      this.bind({ keyup: "keyup", keydown: "keydown" }, (e, type, view2, targetEl, phxEvent, phxTarget) => {
        ...
        js_default.exec(type, phxEvent, view2, targetEl, ["push", { data }]);
      });
      this.bind({ blur: "focusout", focus: "focusin" }, (e, type, view2, targetEl, phxEvent, phxTarget) => {
        if (!phxTarget) {
          let data = __spreadValues({ key: e.key }, this.eventMeta(type, e, targetEl));
          js_default.exec(type, phxEvent, view2, targetEl, ["push", { data }]);
        }
      });
      this.bind({ blur: "blur", focus: "focus" }, (e, type, view2, targetEl, phxEvent, phxTarget) => {
        if (phxTarget === "window") {
        ...
        this.on(browserEventName, (e) => {
          ...
          if (targetPhxEvent) {
            this.debounce(e.target, e, browserEventName, () => {
              this.withinOwners(e.target, (view2) => {
                callback(e, event, view2, e.target, targetPhxEvent, null);
              });
            });
          } else {
            dom_default.all(document, `[${windowBinding}]`, (el) => {
              ...
              this.debounce(el, e, browserEventName, () => {
                this.withinOwners(el, (view2) => {
                  callback(e, event, view2, el, phxEvent, "window");
                });
              });
            });
          }
    ...
    bindClick(eventName, bindingName) {
      ...
      window.addEventListener(eventName, (e) => {
        ...
        this.debounce(target, e, "click", () => {
          this.withinOwners(target, (view2) => {
            js_default.exec("click", phxEvent, view2, target, ["push", { data: this.eventMeta("click", e, target) }]);
          });
        });
      }, false);
    }
        ...
        if (!(el.isSameNode(clickStartedAt) || el.contains(clickStartedAt))) {
          this.withinOwners(el, (view2) => {
            ...
            if (js_default.isVisible(el) && js_default.isInViewport(el)) {
              js_default.exec("click", phxEvent, view2, el, ["push", { data: this.eventMeta("click", e, e.target) }]);
            }
          });
        }
      });
    ...
    bindForms() {
      ...
      this.on("submit", (e) => {
        ...
        this.withinOwners(e.target, (view2) => {
          js_default.exec("submit", phxEvent, view2, e.target, ["push", { submitter: e.submitter }]);
        });
      }, false);
      for (let type of ["change", "input"]) {
        this.on(type, (e) => {
          ...
          this.debounce(input, e, type, () => {
            this.withinOwners(dispatcher, (view2) => {
              dom_default.putPrivate(input, PHX_HAS_FOCUSED, true);
              if (!dom_default.isTextualInput(input)) {
                this.setActiveElement(input);
              }
              js_default.exec("change", phxEvent, view2, input, ["push", { _target: e.target.name, dispatcher }]);
            });
          });
        }, false);
      }
    ...
    debounce(el, event, eventType, callback) {
      ...
      this.withinOwners(el, (view2) => {
        let asyncFilter = () => !view2.isDestroyed() && document.body.contains(el);
  ...
  function clamp(num) {
    return Math.max(0.5, Math.min(num, 40));
  }
  ...
  Hooks2.Canvas = {
    mounted() {
      ...
      this.handleEvent("init_board", ({ data }) => {
        ...
        let completed = 0;
        data.forEach((pixel) => {
          const red = pixel.color >> 16 & 255;
          const green = pixel.color >> 8 & 255;
          const blue = pixel.color & 255;
  ...
  import_topbar.default.config({ barColors: { 0: "#000000" }, shadowColor: "rgba(0, 0, 0, .3)" });
